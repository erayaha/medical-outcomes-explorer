...
export function getAllDeviceMeasureLinks() {
  return loadJson("device_to_measure_map.json");
}

export function getDeviceMeasureLinks(productCode) {
  return getAllDeviceMeasureLinks().find(
...
export function getOutcomeOverview() {
  ...
  return {
    ...
    safetyScores: hospitals.map((hospital) => ({
      period: `${hospital.state} (${hospital.city})`,
...
export function getClinicalTrialFacilities() {
  const hospitals = getHospitals();
  return hospitals.map((hospital) => ({
    ...
    bedCount: hospital.bedCount || 500,
    overallRating: hospital.overallRating,
    clinicalTrialScore: hospital.clinicalTrialScore || 85,
    researchTier: hospital.researchTier || "Tier 2 - Regional Health System",
    specialties: hospital.specialties || ["General Inpatient Care"],
    mortalityBetterCount: hospital.qualityCounts?.mortalityBetter || 0,
    readmissionBetterCount: hospital.qualityCounts?.readmissionsBetter || 0,
    safetyBetterCount: hospital.qualityCounts?.safetyBetter || 0,
  }));
}

export function getDeviceSignalSummary() {
  const devices = getDevices();
  return {
    totalDevices: devices.length,
    therapeuticAreas: [...new Set(devices.map((d) => d.therapeuticArea || d.medicalSpecialty))],
    totalAdverseEvents: devices.reduce(
      (sum, d) => sum + (d.clearanceTimeline?.reduce((acc, t) => acc + (t.events || 0), 0) || 0),
      0,
    ),
    totalRecalls: devices.reduce(
      (sum, d) => sum + (d.clearanceTimeline?.reduce((acc, t) => acc + (t.recalls || 0), 0) || 0),
      0,
    ),
    totalDeathsReported: devices.reduce(
      (sum, d) => sum + (d.eventBreakdown?.deaths || 0),
      0,
    ),
  };
}
